defmodule Sacrum.Repo.CodeRefs do
  ...
  def list_for_task(task_id, user_id) when is_binary(task_id) and is_binary(user_id) do
    from(r in CodeRef,
      where: r.task_id == ^task_id and r.user_id == ^user_id,
      order_by: [asc: r.inserted_at]
    )
    |> Repo.all()
  end
  ...
  def list_for_section(section_id, user_id) when is_binary(section_id) and is_binary(user_id) do
    from(r in CodeRef,
      where: r.section_id == ^section_id and r.user_id == ^user_id,
      order_by: [asc: r.inserted_at]
    )
    |> Repo.all()
  end

  def insert_for_task(%Task{id: task_id, user_id: user_id}, attrs) when is_binary(user_id),
    do: insert_for_task(task_id, user_id, attrs)
  ...
  def insert_for_task(task_id, user_id, attrs) when is_binary(task_id) and is_binary(user_id) do
  ...
  def insert_for_section(%TaskSection{id: section_id, user_id: user_id}, attrs)
      when is_binary(user_id),
      do: insert_for_section(section_id, user_id, attrs)

  def insert_for_section(section_id, user_id, attrs)
      when is_binary(section_id) and is_binary(user_id) do
