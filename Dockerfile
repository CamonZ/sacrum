...
# copy compile-time config for runtime
...
COPY --from=builder --chown=nobody:root /app/_build/${MIX_ENV}/rel/sacrum ./

USER nobody

# If using an environment that doesn't automatically reap zombie processes, it is
# advised to add an pointinit process such as tini via `apt-get install`
